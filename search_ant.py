...
class AntRewardSolver(RewardSolver):
...
def main(args):
    ...
    # 定义全连接矩阵 A 和存在的边 G
    num_agents = 250
    A = np.ones((num_agents, num_agents))  # 全连接矩阵
    # 不连接自己
    np.fill_diagonal(A, 0)
    G = np.array([[i, j] for i in range(num_agents) for j in range(num_agents) if i != j])  # 存在的边

    data['A'] = A
    data['G'] = G

    # init Rewarder
    rewarder = AntRewardSolver(
